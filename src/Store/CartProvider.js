...
function CartProvider(props) {
    ...
    const CartReducer = (state,action)=>{
        if(action.type==='ADD_ITEM'){
            ...
            const checkItemCart = state.items.findIndex(item=>item.id===action.item.id)
        ...
        if(action.type==='REMOVE_ITEM'){
            const checkItemCart = state.items.findIndex(item=>item.id===action.id)
            const indexCartItem =state.items[checkItemCart]; 
            let updateTotalAmount =state.totalAmount;
            let updateItems;
            if(indexCartItem.amount>1){
                updateTotalAmount   = state.totalAmount - indexCartItem.price;
                const updateItem = {...indexCartItem,amount:indexCartItem.amount -1}
                updateItems = [...state.items]
                updateItems[checkItemCart]=updateItem
            }else{
                updateItems = [...state.items]
        ...
        if(action.type==='DELETE_ITEM'){
            ...
            let updateTotalAmount = state.totalAmount-(indexCartItem.amount*indexCartItem.price)
            return{
                items:updateItems,
                totalAmount:updateTotalAmount
            }
